...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  ...
  | Replicate (n_exp, a_exp, el_tp, (line, _)) ->
      ...
      let a_code = compileExp a_exp vtable a_reg  // evaluating a_exp and place result in a_reg

      let addr_reg = newName "addr_reg"
      let i_reg = newName "i_reg"   //counter
      let init_regs = [ Mips.ADDI (addr_reg, place, "4")  //addr_reg = place+4 
                                                          //(since first place in array 
                                                          //holds the int value of the size 
                                                          //(int is of size 4byte))
                      ; Mips.MOVE (i_reg, "0") ]          //set counter to 0

      let loop_beg = newName "loop_beg"
      let loop_end = newName "loop_end"
      let tmp_reg = newName "tmp_reg"
      let loop_header = [ Mips.LABEL (loop_beg)               //loop_beg:     
                        ; Mips.SUB (tmp_reg, i_reg, size_reg) //tmp = i - size  
                        ; Mips.BGEZ (tmp_reg, loop_end)       //if tmp >= 0 goto END
                        ; Mips.ADDI (i_reg, i_reg, "1")       //i_reg + 1 (counter one up)
                        ]
      let loop_replicate = 
                        match getElemSize el_tp with
                          | One -> [Mips.SB (a_reg, addr_reg, "0");       //if el. size 1, store byte
                                   Mips.ADDI (addr_reg, addr_reg, "1") ]  //next addres-space 1 ahead
                          | Four -> [ Mips.SW (a_reg, addr_reg, "0");     //if el. size 4, store word
                                     Mips.ADDI (addr_reg, addr_reg, "4")] //next addres-space 4 ahead 
      let loop_footer =   
                        [ Mips.J loop_beg                      //goto loop_beg 
                        ; Mips.LABEL loop_end                  //END
                        ]
      n_code
       @ a_code
       @ checksize
       @ dynalloc (size_reg, place, el_tp)
       @ init_regs
       @ loop_header
       @ loop_replicate
       @ loop_footer

  | Map (farg, arr, tp1, tp2, pos) ->
      let size_reg = newName "size_reg"  //register for holding size of array
      ...
      let arr_code = compileExp arr vtable arr_reg     // evaluating arr and place result in arr_reg 
      let header1 = [ Mips.LW(size_reg, arr_reg, "0")] // the first index in arr holds the lenghth
                                                       // size_reg is set to the lenght of  the array
      let loop_beg = newName "loop_beg"
      let loop_end = newName "loop_end"
      let loop_code =
              [ Mips.ADDI(arr_reg, arr_reg, "4")      //arr_reg = arr_reg+4 (since first element was int)
              ; Mips.ADDI(r_reg, place, "4")          //r_reg = place+4
              ; Mips.MOVE(i_reg, "0")                 //counter set to 0
              ; Mips.LABEL(loop_beg)                  //loop_beg:
              ; Mips.SUB(tmp_reg, i_reg, size_reg)    //tmp_reg = counter - size
              ; Mips.BGEZ(tmp_reg, loop_end)          // if tmp >= 0 then goto END 
              ]

      let load_code =
              match getElemSize tp1 with
                | One  -> [ Mips.LB   (tmp_reg, arr_reg, "0") //if el. size 1, load byte
                          ; Mips.ADDI (arr_reg, arr_reg, "1") //next space to load 1 ahead
                          ]
                | Four -> [ Mips.LW   (tmp_reg, arr_reg, "0") //if el. size 4, load byte
                          ; Mips.ADDI (arr_reg, arr_reg, "4") //next space to load 4 ahead
                          ]

      let save_code = 
              match getElemSize tp2 with
                | One -> [ Mips.SB (tmp_reg, r_reg, "0")  //if el. size 1, store byte
                         ; Mips.ADDI (r_reg, r_reg, "1")  //next space to store 1 ahead
                         ] 

                | Four -> [ Mips.SW (tmp_reg, r_reg, "0") //if el. size 4, store byte
                          ; Mips.ADDI (r_reg, r_reg, "4") //next space to store 4 ahead
                          ]      

      let apply_code =
          applyFunArg(farg, [tmp_reg], vtable, tmp_reg, pos) //applys function to tmp_reg
                                                             //tmp_reg set to the result

      arr_code @  header1 @ dynalloc (size_reg, place, tp1) @ loop_code @ load_code @ apply_code @ save_code @
         [ Mips.ADDI(i_reg, i_reg, "1") //i_reg +1 (counter 1 up)
         ; Mips.J loop_beg              //goto loop_beg
         ; Mips.LABEL loop_end          //END
         ]
  ...
  | Scan (binop, acc_exp, arr_exp, tp, pos) ->
      ...
      let acc_reg = newName "acc_reg"   //accumulate_register


      let arr_code = compileExp arr_exp vtable arr_reg //evaluating arr_exp and place result in arr_reg 

      let init_regs = [ Mips.MOVE (i_reg, "0")             //counter set to 0
                      ; Mips.LW(size_reg, arr_reg, "0")    //the first index in arr holds the lenghth
                                                           //size_reg is set to the lenght of  the array
                      ; Mips.ADDI(size_reg, size_reg, "1")   //size_reg = size_reg+1 
                      ; Mips.ADDI(arr_reg, arr_reg, "4")]    //arr_reg = arr_reg+4 (since first element w/ size is int)
                      @ dynalloc (size_reg, place, tp)       //array place lenghth size type tp
                      @[ Mips.ADDI (r_reg, place, "4")]     //result pointer set to place+4
      ...
      let apply_code = //applys function to tmp_reg,  tmp_reg set to the result
      ...
      let loop_footer = 
               [ Mips.ADDI(i_reg, i_reg, "1") //counter one up
               ; Mips.J loop_beg              //goto loop_beg
               ; Mips.LABEL loop_end          //END
               ]               
