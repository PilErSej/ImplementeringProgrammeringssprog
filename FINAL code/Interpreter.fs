...
(* Timess, Divide, Not and Negate *)
  ...
  | Not(e1, pos) ->    
        let res1   = evalExp(e1, vtab, ftab)   //evaluates e1
        match res1 with
          | BoolVal true -> BoolVal false     //if res1 of e1 is true then false
          | BoolVal false -> BoolVal true     //if res1 of e1 is false then true
          | _ -> invalidOperand "Not on non-boolean arg: " Bool res1 pos

  | Negate(e1, pos) ->                                       
        let res1   = evalExp(e1, vtab, ftab) //evaluates e1
        match res1 with
          | IntVal n1 -> IntVal (n1*(-1))  //if res1 is of type IntVal then multiply w/ -1 
...
(* And and Or *)
  | And (e1, e2, pos) ->
        ...
        match r1 with
          | BoolVal true ->  let r2 = evalExp(e2, vtab, ftab) //if true then evaluate e2
                             match r2 with  //the evaluation of e2 decides true or false
                                | BoolVal n1 ->  BoolVal n1  
                                | _ ->  invalidOperand "Invalid And operand types" Bool r2 pos

          | BoolVal false ->  BoolVal false  //if r1 of e1 is false then false
  ...
  | Or (e1, e2, pos) ->
        ...
        match r1 with
          ...
          | BoolVal true ->  BoolVal true  //if r1 is true then true
  ...
  | Reduce (farg, ne, arrexp, tp, pos) ->
        ...
        match arr with
          ...
          | otherwise -> raise (MyError("Third argument of reduce is not an array: "+ppVal 0 arr
                                       , pos))
  (* Replicate, Map and Scan *)


  | Replicate (n, a , el_type, pos) ->
        let n1 = evalExp(n, vtab, ftab) //evaluates n
        match n1 with                   
          | IntVal size ->
              if (size >= 0) then    //chek if n1 is type IntVal >=0
                  let a1 = evalExp(a, vtab, ftab)      //evaluates a
                  let mutable tp = Int  //tp is set by default to Int
                  match a1 with //sets tp the Type corresponding to Value of a1
                    | IntVal _  -> tp <- Int 
                    | BoolVal _ -> tp <- Bool
                    | CharVal _ -> tp <- Char
                    | (ArrayVal (_,tp1)) -> tp <- Array tp1 
                  ArrayVal( List.map (fun x -> a1 ) [0..size-1], tp ) //array of type tp
                                                                      //with a1 (size) times
  ...
  | Map (farg, arrexp, tp1, tp2, pos) ->
        let farg_ret_type = rtpFunArg farg ftab pos // function return type
        let arr  = evalExp(arrexp, vtab, ftab)      // evaluate arrexp
        match arr with
        | ArrayVal(lst, _) -> //if arrexp is Array then 
                let newlst = List.map(fun x -> evalFunArg (farg, vtab, ftab, pos, [x])) lst
                ArrayVal(newlst,Array(farg_ret_type))
                //map x in list on fuction into newlst
                //Array of newlst  with type Array of the functions return type
        | _ -> raise (MyError("argument of map is not an array: "+ppVal 0 arr, pos))


  | Scan (farg, ne, arrexp, tp, pos) ->
        let farg_ret_type = rtpFunArg farg ftab pos // function return type
        let arr  = evalExp(arrexp, vtab, ftab)      // evaluate arrexp
        let nel  = evalExp(ne, vtab, ftab)          // evaluate ne
        match arr with   //chek arr is an array
          | ArrayVal (lst,tp1) ->
