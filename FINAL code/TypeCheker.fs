...
(* Times, Divide, Not and Negate *)
  ...
  (* AND and OR *)
    ...
    | Replicate (n, a, _, pos) ->
        let (t_n, n_dec) = checkExp ftab vtab n   //finds type of n
        if (Int = t_n) then                       //cheks if the type of n is Int
          let (t_a, a_dec) = checkExp ftab vtab a //finds type of a
          (Array t_a, Replicate (n_dec, a_dec, t_a, pos)) //return array type t_a
                                                          //replicate call with elem_type t_a
        else raise (MyError ("Cannot Replicate with type " + ppType t_n, pos))

    | Map (f, arr_exp, _, _, pos) -> 
        let (arr_type, arr_dec) = checkExp ftab vtab arr_exp  //finds type of arr_exp
        let elem_type = //sets elem_type to the type of elements in array arr 
            match arr_type with
              | Array t -> t
        ...
        let (f', f_in, f_out)= //find types of function f
            match checkFunArg ftab vtab pos f with 
              ...
              | (_, res, args) ->
                  raise (MyError ( "Map2: incompatible function type of " +
                                   ppFunArg 0 f + ": " + showFunType (args, res)
                                 , pos))
        (Array f_out, Map (f', arr_dec, elem_type, f_out, pos))  //return array type f_out
                                                                 //Map call with elem_type and f_out

    | Scan (f, n_exp, arr_exp, _, pos) ->
        let (n_type  , n_dec  ) = checkExp ftab vtab n_exp    //finds type of n_exp
        let (arr_type, arr_dec) = checkExp ftab vtab arr_exp  //finds type of arr_exp
        ...
        if   elem_type = f_arg_type && elem_type = n_type then     //elem_type = the same as the f's result type
             (arr_type, Scan (f', n_dec, arr_dec, elem_type, pos)) //return type is same as arr_type
                                                                   //Scan call with elem_type 
        elif elem_type = f_arg_type then
             raise (err ("neutral element", n_type))
        else raise (err ("array element", elem_type))
