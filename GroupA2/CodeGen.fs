...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  | And (e1, e2, pos) ->      
       ...
       else code1 @ [Mips.ANDI (place,t1, "1")]           //make Mips instruction list with a false Immediate (0)
  ...
  *)
  | Replicate (n_exp, a_exp, el_tp, (line, _)) ->
      ...
      *)
      let safe_lab = newName "safe_lab"
      let checksize = [ Mips.ADDI (size_reg, size_reg, "-1")   // sizereg = sizereg-1
                      ; Mips.BGEZ (size_reg, safe_lab)        //sizereg >= 0 jump to safe_lab
                      ; Mips.LI ("5", makeConst line)         //?
                      ; Mips.J "_IllegalArrSizeError_"        //jump to error
                      ; Mips.LABEL (safe_lab)                 //<-- here to
                      ; Mips.ADDI (size_reg, size_reg, "1")   //sizereg = sizereg+1
                      ]
      ...
      let loop_header = [ Mips.LABEL (loop_beg)               
                        ...
                        ; Mips.BGEZ (tmp_reg, loop_end)       //if tmp >= 0 tne end
                        ; Mips.ADDI (i_reg, i_reg, "1")        //i_reg + 1
                        ]
      let loop_replicate = 
                        match getElemSize el_tp with
                          | One -> [Mips.SB (a_reg, addr_reg, "0"); 
                                   Mips.ADDI (addr_reg, addr_reg, "1") ] 
                          | Four -> [ Mips.SW (a_reg, addr_reg, "0"); 
                                     Mips.ADDI (addr_reg, addr_reg, "4")  ]      //save word t 
      let loop_footer =   //addr = addr+4
                        [ Mips.J loop_beg                      //loop 
  ...
  | Map (farg, arr, tp1, tp2, pos) ->
      ...
      let save_code = 
              match getElemSize tp2 with
                | One -> [ Mips.SB (tmp_reg, r_reg, "0")
                         ; Mips.ADDI (r_reg, r_reg, "1") 
                         ] 

                | Four -> [ Mips.SW (tmp_reg, r_reg, "0")
                          ; Mips.ADDI (r_reg, r_reg, "4")  
                          ]      
